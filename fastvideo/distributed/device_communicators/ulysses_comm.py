...
# (PR #3820, "Feat/ulysses p2p a2a"), file: comm/ulysses.py
#
# Copied whole and deliberately not trimmed. The staged init/teardown and the
# group-wide outcome exchange in UlyssesCommunicator are what stop one rank's
# failure from hanging its peers; editing them to remove paths we happen not to
# use is exactly how that property gets broken. Unused code here is free.
#
# Substitutions (each marked VENDOR SUBSTITUTION at its site):
#   - flashinfer_api telemetry decorator -> local no-op
#   - JIT module build -> prebuilt fastvideo_kernel.comm_ops
#   - vllm_ar.meta_size -> comm_ops.signal_size
#   - dropped: register_custom_op (upstream wraps its ops for torch.compile;
#     our callers run under torch.compiler.disable)
#
# Do not edit beyond those -- re-sync from upstream.
...
def flashinfer_api(fn):
    """No-op stand-in for flashinfer's telemetry decorator (VENDOR SUBSTITUTION)."""
    return fn
...
class UlyssesCommunicator:
    ...
    def _nvlink_init_transaction(self) -> Optional[str]:
        ...
        try:
            from fastvideo_kernel.comm_ops import signal_size as meta_size  # VENDOR SUBSTITUTION
...
def get_ulysses_a2a_module():
    """VENDOR SUBSTITUTION: the kernel is compiled into fastvideo-kernel.

    Upstream JIT-compiles here and wraps each entry point in a torch custom op.
    We ship the kernel prebuilt in the fastvideo-kernel wheel instead, so there
    is nothing to build and no compiler needed at runtime -- which is the whole
    reason for vendoring. The attribute names below are what the validating
    wrappers further down this file expect, so everything past this point is
    unmodified upstream code.
    """
    from fastvideo_kernel import comm_ops

    return SimpleNamespace(
        init_ulysses_a2a=comm_ops.init,
        dispose_ulysses_a2a=comm_ops.dispose,
        ulysses_a2a=comm_ops.all_to_all,
    )
